...
def download_file(path_from, path_to):
    # path_from       http://10.1.0.228/mirror/dir/690870/fucking.pdf
    # path_to         /var/www/srv-spb-010/sap/10.1.0.228/dir/690870/fucking.pdf
    global local_path
    global url
    ...
    try:
      with urllib.request.urlopen(path_from) as response, open(path_to, 'wb') as file_download:
        ...
        print(path_to)
        #print(path_from.encode('unicode-escape').decode())
        #print('from', repr(path_from))
        response = ''

def folders_links(root_folder):
    # root_folder        http://10.1.0.228/mirror/bel/
    folders = []
    ...
    for link in links.findAll('a')[1:]:
        ...
        #yield root_folder + sub_folder + '/'
    return folders

def files_links(folder):
    # folder: http://10.1.0.228/mirror/bel/700493/
    files = []
    global local_path
    global period
...
# download_file(path_from, path_to)
# path_from       http://10.1.0.228/mirror/dir/690870/fucking.pdf
# path_to         /var/www/srv-spb-010/sap/10.1.0.228/dir/690870/fucking.pdf

#   folder                                       file
# http://10.1.0.228/mirror/dir/690870/     /dir/690870/fucking.pdf
#
#  url = 'http://10.1.0.228/mirror'
#  local_path = '/var/www/srv-spb-010/sap/10.1.0.228'
...
#    mt = threading.Thread(target=download_file, args=(c_file, const_local_path + os.path.basename(c_file),))
#    mt.start()
    ...
    try:
        ...
        for folder in folders_links(full_path):
            for file in files_links(folder):
                ...
                download_file(url + file, local_path + file)
                #mt = threading.Thread(target=download_file, args=(url + file, local_path + file))
                #mt.start()
    except Exception as e:
        print(e)
        print('dir', dir)
