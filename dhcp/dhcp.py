...
def ping_fixed(fixed_file):
        ...
        for line in fixed_file:
                ...
                key = tokens[5].lower()
                host = tokens[1].lower()
                ip = re.findall(r'[0-9]+(?:\.[0-9]+){3}', key)
                ip_str = ''.join(ip)
                response = os.system("ping -c 1 " + ip_str + ">/dev/null")
                if response == 0:
                        fixed.append(ip_str)
                        fixed.append(host)
                else:
                        continue
                fixed_db.append(fixed)
...
try:
    if sys.argv[1] == "-a":
        fixfile = open('/etc/dhcp/dhcpd-fixed.conf', 'r')
        fixed = ping_fixed(fixfile)
        fixfile.close()
